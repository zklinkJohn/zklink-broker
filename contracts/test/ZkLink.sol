...
contract ZkLink is IZkLink {
  mapping(uint16 => address) public tokenIdMap;
  event Accept(address from, address to, uint128 amount);

  function setTokenId(uint16 tokenId, address addr) external {
    tokenIdMap[tokenId] = addr;
  }
  ...
  function acceptETH(
    ...
    uint128 amountTransfer = (amount * (10000 - withdrawFeeRate)) / 10000;

    require(amount >= amountTransfer, 'must gt');
    if (amount > amountTransfer) {
      acceptor.call{value: amount - amountTransfer}('');
    }
    receiver.call{value: amountTransfer}('');
  }
  ...
  function acceptERC20(
    ...
    uint128 _amountTransfer = (amount * (10000 - withdrawFeeRate)) / 10000;
    // require(amountTransfer == _amountTransfer, 'must eq');
    require(msg.sender == acceptor, 'must eq2');
    IERC20 token = IERC20(tokenIdMap[tokenId]);

    token.transferFrom(msg.sender, receiver, amountTransfer);
    emit Accept(acceptor, receiver, amountTransfer);
  }
